...
Спасибо, что помогаете улучшать `@trucklinemp/sdk`, официальный набор инструментов TypeScript и JavaScript для [Публичного API](/ru/web-api/public-api/) TrucklineMP, OAuth и вебхуков.

Это руководство предназначено для тех, кто хочет изменить сам **пакет SDK** (новые методы, исправления ошибок, типы, документация в репозитории). Если вам нужно только **использовать** SDK в приложении, вместо этого ознакомьтесь с разделом [TypeScript SDK](/ru/web-api/sdk/).
...
2. Проверьте [спецификацию OpenAPI](https://trucklinemp.com/api/v1/openapi.json) и [руководство по публичному API](/ru/web-api/public-api/), чтобы узнать реальный HTTP-контракт. SDK — это тонкий клиент, он должен отражать публичные маршруты, а не выдумывать приватные.
...
4. Обновите страницу пользовательской документации ([TypeScript SDK](/ru/web-api/sdk/)), если изменение затрагивает пользователей.
...
| Ресурс | Ссылка |
|----------|------|
| Использование SDK | [TypeScript SDK](/ru/web-api/sdk/) |
| Публичный API | [Публичный API](/ru/web-api/public-api/) |
| Вебхуки | [Вебхуки](/ru/web-api/webhooks/) |
...
| Перевод документации | [Перевод документации](/ru/web-docs/contribute/contributing/) |
