...
Do **not** append an extra `/v1` on `api.trucklinemp.com`. See [Public API base URLs](/web-api/public-api/#base-urls).
...
Platform limit tables: [Rate limits](/web-api/public-api/#rate-limits).

## OAuth

Use OAuth when you need data **as a signed-in user** (not with an API key alone). Full product flow: [OAuth Apps](/web-api/oauth-apps/).
...
Redirect URI rules (localhost / private LAN / https): [OAuth Apps - redirect URIs](/web-api/oauth-apps/#redirect-uri-rules).
...
Event catalog and retries: [Webhooks](/web-api/webhooks/).
...
API traffic with your key is subject to the same logging and rate limiting as raw HTTP. See the [Privacy Policy](https://id.trucklinemp.com/legal/privacy) and [Public API](/web-api/public-api/) telemetry notes.

## Contributing

Package layout, PRs, and adding methods: [Contributing to the TypeScript SDK](/web-api/sdk-contributing/).

## Related guides

| Guide | Topics |
|-------|--------|
| [Platform Overview](/web-api/overview/) | Developer Mode, integration types |
| [Public API](/web-api/public-api/) | Auth, base URLs, rate limits, endpoint map |
| [OAuth Apps](/web-api/oauth-apps/) | Authorize, tokens, scopes, redirect URIs |
| [Webhooks](/web-api/webhooks/) | Events, signatures, retries |
| [Developer Console](/web-api/console/) | Projects, keys, playground |
| [Contributing to the SDK](/web-api/sdk-contributing/) | Local setup and PRs |
| [Leaked API Keys & Secrets](/web-api/leaked-secrets/) | Rotation after exposure |
